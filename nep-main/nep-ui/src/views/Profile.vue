...
import { 
  Timer, User, Iphone, Lock, Message, 
...
function saveAvatar(url) {
  // 仅记录头像 URL，不立即保存；用户点击"保存修改"时统一提交
  avatar.value = url
}
...
.avatar-hint { font-size: 12px; color: #A0AAB2; font-weight: 500; text-align: center; }
...
/* 分段控制器 (Radio Group) */
.swiss-segmented-control { display: flex; width: 100%; background: rgba(28, 36, 33, 0.04); padding: 3px; border-radius: 14px; box-sizing: border-box; gap: 4px; }
.swiss-segmented-control :deep(.el-radio-button) { flex: 1; margin-left: 0 !important; border-radius: 11px; overflow: hidden; }
.swiss-segmented-control :deep(.el-radio-button__inner) { width: 100%; border: none !important; background: transparent !important; box-shadow: none !important; border-radius: 11px !important; color: #74807B; font-weight: 600; padding: 10px 0; transition: all 0.3s; }
.swiss-segmented-control :deep(.el-radio-button.is-active .el-radio-button__inner) { background: white !important; color: #1C2421; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.06) !important; }
.swiss-segmented-control :deep(.el-radio-button:first-child .el-icon) { color: #3B82F6; }
.swiss-segmented-control :deep(.el-radio-button:last-child .el-icon) { color: #EC4899; }
...
<style>
/* 强制覆盖 Element Plus radio-button 默认 -1px 负边距，让男女按钮分开 */
.swiss-segmented-control .el-radio-button { margin-left: 0 !important; }
</style>