            ...
            <div class="col-action" @click.stop>
              ...
              <template v-if="row.status === 'PENDING' || row.status === 'ESCALATED'">
                <el-select v-model="row._inspectorName" :placeholder="row.status === 'ESCALATED' ? '紧急指派' : '选择网格员'"
                           size="small" class="inline-select">
                  <el-option v-for="ins in inspectorOptions" :key="ins.id"
                ...
                <div class="action-btn-group">
                  ...
                  <button class="recommend-btn" title="智能推荐" @click="openRecommend(row)">
...
/* 操作区 */
.col-action { display: flex; align-items: center; gap: 12px; justify-content: flex-end; position: relative; padding-right: 34px; }
.inline-assign { display: flex; align-items: center; gap: 6px; }
.action-done { font-size: 12px; font-weight: 500; color: #A0AAB2; padding: 0 8px; white-space: nowrap; }
...
/* 网格员选择框加宽 */
.inline-select .el-select__wrapper { min-width: 140px; }
