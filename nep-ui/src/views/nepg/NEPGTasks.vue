...
import { useRouter, useRoute } from 'vue-router' // 核心修改：引入 useRoute
...
const route = useRoute() // 核心修改：初始化 route 以获取 URL 参数
...
const fetchAssignedTasks = async () => {
  ...
  try {
    ...
    // 核心修改：在这里读取 URL 的 taskId 并在数据加载完后自动选中该任务
    const queryTaskId = route.query.taskId
    if (queryTaskId) {
      // 通过转换为字符串对比，避免类型不匹配（路由参数通常是 string）
      const targetTask = rawTasks.value.find(t => String(t.id) === String(queryTaskId))
      if (targetTask) {
        currentTab.value = 'all' // 确保在全部列表里能看到
        selectTask(targetTask)   // 自动选中任务触发详情展示
        
        // 可选项：选中后清除路由栏的 ?taskId 参数，保持 URL 整洁
...
<style scoped>
/* 样式无需改动，完全保留您的代码 */
.alpine-tasks-canvas { width: 100%; height: 100%; display: flex; flex-direction: column; box-sizing: border-box; overflow: hidden; }
.tasks-header { flex-shrink: 0; display: flex; justify-content: space-between; align-items: flex-end; padding-bottom: 24px; border-bottom: 1px solid rgba(15, 23, 42, 0.06); margin-bottom: 24px; }
...
.alpine-search { display: flex; align-items: center; gap: 8px; background: white; border: 1px solid rgba(15, 23, 42, 0.08); padding: 8px 16px; border-radius: 12px; width: 280px; transition: all 0.3s; }
...
.alpine-segments { display: flex; background: rgba(15, 23, 42, 0.04); padding: 4px; border-radius: 12px; }
.segment-btn { border: none; background: transparent; padding: 6px 16px; border-radius: 8px; font-size: 13px; font-weight: 600; color: #64748B; cursor: pointer; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); }
.segment-btn.active { background: white; color: #0F172A; box-shadow: 0 2px 8px rgba(15, 23, 42, 0.06); }
.tasks-workspace { flex: 1; display: flex; gap: 24px; min-height: 0; }
.task-list-pane { width: 380px; flex-shrink: 0; display: flex; flex-direction: column; gap: 12px; overflow-y: auto; padding-right: 8px; }
...
.task-card { background: white; border-radius: 16px; padding: 20px; border: 1px solid rgba(15, 23, 42, 0.04); cursor: pointer; transition: all 0.2s ease; display: flex; flex-direction: column; gap: 12px; }
...
.task-detail-pane { flex: 1; background: white; border-radius: 20px; border: 1px solid rgba(15, 23, 42, 0.04); box-shadow: 0 4px 24px -8px rgba(15, 23, 42, 0.03); display: flex; flex-direction: column; overflow: hidden; }
.detail-content { flex: 1; display: flex; flex-direction: column; min-height: 0; }
.detail-scroll-area { flex: 1; overflow-y: auto; padding: 40px; }
...
.map-placeholder { margin-top: 16px; height: 120px; border-radius: 12px; background: rgba(14, 165, 233, 0.03); border: 1px dashed rgba(14, 165, 233, 0.2); display: flex; flex-direction: column; justify-content: center; align-items: center; gap: 8px; color: #0284C7; font-size: 13px; font-weight: 500; }
...
.detail-actions { flex-shrink: 0; padding: 24px 40px; background: rgba(255, 255, 255, 0.9); backdrop-filter: blur(10px); border-top: 1px solid rgba(15, 23, 42, 0.04); display: flex; justify-content: flex-end; gap: 16px; }
.alpine-btn { padding: 12px 24px; border-radius: 12px; font-size: 14px; font-weight: 600; display: inline-flex; align-items: center; justify-content: center; gap: 8px; cursor: pointer; border: none; transition: all 0.3s; }
...
.glass-placeholder { width: 80px; height: 80px; border-radius: 24px; margin-bottom: 24px; background: linear-gradient(135deg, #F0F9FF 0%, #E0F2FE 100%); display: flex; justify-content: center; align-items: center; font-size: 32px; color: #0284C7; box-shadow: 0 12px 32px rgba(2, 132, 199, 0.1); }
...
.aqi-drawer-content { display: flex; flex-direction: column; height: 100%; }
.aqi-form-section { flex: 1; overflow-y: auto; padding-bottom: 24px; }
.form-section-title { display: flex; align-items: center; gap: 8px; font-size: 16px; font-weight: 600; color: #0F172A; margin: 0 0 8px 0; }
.form-hint { font-size: 13px; color: #64748B; margin: 0 0 24px 0; }
.aqi-input-group { margin-bottom: 20px; }
.aqi-label { display: block; font-size: 14px; font-weight: 500; color: #0F172A; margin-bottom: 8px; }
.aqi-preview-card { background: linear-gradient(135deg, #F8FAFC 0%, #F1F5F9 100%); border: 2px solid #E2E8F0; border-radius: 16px; padding: 24px; text-align: center; margin: 24px 0; }
.preview-label { font-size: 12px; color: #64748B; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px; }
.preview-value { font-size: 48px; font-weight: 700; line-height: 1; margin-bottom: 8px; }
.preview-level { font-size: 18px; font-weight: 600; margin-bottom: 4px; }
.preview-hint { font-size: 12px; color: #94A3B8; font-family: monospace; }
.drawer-actions { padding-top: 16px; border-top: 1px solid rgba(15, 23, 42, 0.06); display: flex; gap: 12px; justify-content: flex-end; }
</style>